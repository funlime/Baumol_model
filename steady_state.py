...
def prepare_hh_ss(model):
    ...
    if par.sigma != 1:
        v_a = (1+ss.r) * par.alpha**(1/par.sigma)*c**(-1/par.sigma)   * (par.alpha**(1/par.sigma)*c**((par.sigma-1)/par.sigma) + (1-par.alpha)**(1/par.sigma)*ss.G**((par.sigma-1)/par.sigma))**((1-par.sigma*par.rho)/(par.sigma-1))
         ...
         v_a = (1+ss.r) * par.alpha * (1/ (par.alpha**par.alpha * (1-par.alpha)**(1-par.alpha))) **(1-par.rho) * c**(par.alpha*(1-par.rho) - 1) * ss.G**((1-par.alpha)*(1-par.rho))
...
def obj_ss(x,model,do_print=False):
    ...
    # print(f'Solving HH for r = {ss.r:6.3f} and wt = {ss.wt:6.3f} and G = {ss.G}')
    ...
    ss.L_Y = ss.L - ss.L_G
    ss.K = KL*ss.L_Y
    # print(f'KL = {KL:6.3f} and K = {ss.K:6.3f} and L = {ss.L:6.3f} and L_Y = {ss.L_Y:6.3f} ')
    ...
    # ss.G_share = ss.G/(ss.Y+ss.G)
    # ss.G_exp_share = G_exp/(G_exp + ss.L_Y*ss.w + ss.K*ss.rK)
    ...
    # print(f'clearing_A = {ss.clearing_A:6.3f} and clearing_L = {ss.clearing_L:6.3f} and clearing_Y = {ss.clearing_Y:6.3f} and clearing_G = {ss.clearing_G:6.3f} ')
...
#    res = root_finding_2.newton_secant(obj_ss, initial_guess, args=(model,), tol=1.48e-08, maxiter=50, disp=True)
    ...
    # print(f'KL = {res.x[0]:6.3f} and tau = {res.x[1]:6.3f} ')
